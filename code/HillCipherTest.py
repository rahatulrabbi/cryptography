...
def matrix_mod_inv(matrix, modulus):
    ...
    det = int(np.round(np.linalg.det(matrix)))  # Step 1) i.e = 9
    det_inv = egcd(det, modulus)[1] % modulus  # Step 2) 1/9 = 3
    matrix_modulus_inv = (
            det_inv * np.round(det * np.linalg.inv(matrix)).astype(int) % modulus
    )  # Step 3) # .astype(int)  convert to integers

    return matrix_modulus_inv # Result of K^-1
